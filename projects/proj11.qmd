...
[Accept the assignment](){target="_blank"} and use `git clone` to create your copy of the code.
...
The height of a BST determines the speed of operations like `insert()` and `contains()`. If we build a "large" BST of N items (e.g., $\sim 2^{20}$), and that tree is balanced, then its height should be the minimal $O(lg(N))$ (e.g., 20).
